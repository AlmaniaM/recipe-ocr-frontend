...
/**
 * Local LLM Recipe Parser
 * 
 * Implementation using local LLM (Ollama) for recipe parsing.
 * This provides privacy-focused recipe parsing without external API calls.
 */
@injectable()
export class LocalLLMRecipeParser implements IRecipeParser {
  ...
  async parseRecipe(text: string): Promise<Result<Recipe>> {
    try {
      ...
      console.error('Local LLM recipe parsing failed:', error);
      return Result.failure(`Local LLM parsing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getParsingConfidence(): Promise<Result<number>> {
    // Local LLM typically has good confidence for recipe parsing
